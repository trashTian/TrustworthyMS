...
logging.basicConfig(
    ...
    filename='ablation_reg_wo_EBUQ.txt',  # 日志文件名
...
def train_epoch(model, device, train_loader, optimizer, criterion):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        output, x_g_0, x_g_1, output1 = model(
            data.x,
            data.edge_index,
            data.batch,
            data.x1.detach(),
        ...
        loss_1 = criterion(output, data.y.view(-1, 1))
        loss_2 = criterion(output1, data.y.view(-1, 1))

        # 总损失
        loss = loss_1 + 0.3 * cl_loss + loss_2
        loss += loss.item()

        preds.append(output.view(-1).cpu())
        labels.append(data.y.float().cpu())
